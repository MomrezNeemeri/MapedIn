...
function Home() {
  ...
  const fetchJobs = async (searchTerm) => {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:5001/api/search', { params: { q: searchTerm } });
      setJobs(res.data);
  ...
  const visibleJobs = jobs.filter(job => {
    ...
    return (job.lat <= bounds.getNorth() && job.lat >= bounds.getSouth() && job.lng <= bounds.getEast() && job.lng >= bounds.getWest());
  });
      ...
      <div className="sidebar">
        <div className="search-area">
          ...
          <div style={{ display: 'flex', gap: '5px' }}>
            ...
            />
            <button onClick={() => fetchJobs(query)} style={{ background: '#003594', color: 'white', border: 'none', borderRadius: '4px' }}>Go</button>
          </div>
        </div>
        <div className="results-list">
          {visibleJobs.map(job => (
            ...
            >
              ...
              <div style={{ fontSize: '0.8rem', color: '#666' }}>📍 {job.city}</div>
            </div>
          ))}
        </div>
      </div>
